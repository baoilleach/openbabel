...
//         obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obError);
  ...
  // Functor for use with replace_if below
  class equal_to_atomId
  {
    public:
      equal_to_atomId(OBStereo::Ref ref): _ref(ref) {};
      OBStereo::Ref _ref;
      bool operator() (OBStereo::Ref ref) {
        return ref == _ref;
      }
  };
  ...
  void StereoRefToImplicit(OBMol& mol, OBStereo::Ref atomId) {
    ...
    for (std::vector<OBGenericData*>::iterator data = vdata.begin(); data != vdata.end(); ++data) {
      ...
      if (datatype == OBStereo::CisTrans) {
        ...
        replace_if(ct_cfg.refs.begin(), ct_cfg.refs.end(), equal_to_atomId(atomId), (OBStereo::Ref) OBStereo::ImplicitRef);
        ct->SetConfig(ct_cfg);
      }
        ...
        replace_if(ts_cfg.refs.begin(), ts_cfg.refs.end(), equal_to_atomId(atomId), (OBStereo::Ref) OBStereo::ImplicitRef);
        ts->SetConfig(ts_cfg);
      }
    }
  }
      ...
      if (datatype == OBStereo::CisTrans) {
        ...
        if (ct_cfg.begin == centerId || ct_cfg.end == centerId) {
          ...
          if (ct_cfg.begin == centerId)
            replace_if(ct_cfg.refs.begin(), ct_cfg.refs.begin()+2, equal_to_atomId(OBStereo::ImplicitRef), (OBStereo::Ref) newId);
          if (ct_cfg.end == centerId)
            replace_if(ct_cfg.refs.begin()+2, ct_cfg.refs.end(), equal_to_atomId(OBStereo::ImplicitRef), (OBStereo::Ref) newId);
          ct->SetConfig(ct_cfg);
        }
      }
      else if (datatype == OBStereo::Tetrahedral) {
        ...
        if (ts_cfg.center == centerId) {
          ...
          replace_if(ts_cfg.refs.begin(), ts_cfg.refs.end(), equal_to_atomId(OBStereo::ImplicitRef), (OBStereo::Ref) newId);
          ts->SetConfig(ts_cfg);
        }
      }
    }
